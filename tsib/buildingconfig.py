...
HEAT_TECHS = ['Oil boiler', 'Gas boiler', 'Heat pump', 'Pellet boiler',
              'Electric heater', 'District heating']

DATA_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'data')

TOTAL_PROFILE_NUM = 20

KWARG_TYPES = {'n_storey': 'NOT_IMPLEMENTED',  # number of storeys in the building
               ...
               'surrounding': ['Detached', 'Semi', 'Terraced'],  # if the house is standing alone or is surrounded
               'ID': str,  # direct identification of an IWU building by its ID
               'buildingYear': int,  # construction year
               'buildingType': ['AB', 'SFH', 'MFH', 'TH'],  # type of building (can be inherited by n_apartments)
               ...
               'existingHeatSupply': ['Oil boiler', 'Gas boiler', 'Heat pump', 'Pellet boiler',
                                      'Electric heater', 'CHP', 'District heating'],
...
KWARG_DEFAULTS = {'roofOrientation': 135.,  # roof azimuth with 180 as south
                  ...
                  'comfortT_lb': 21.,  # lower bound of the comfortable temperature if active
                  'comfortT_ub': 24.,  # upper bound of the comfortable temperature if active
...
class BuildingConfiguration(object):
        ...
        for kwarg in list(kwargs):
            ...
            # drop not defined keywords from dictionary
                ...
                if KWARG_TYPES[kwarg] is 'NOT_IMPLEMENTED':
                    raise NotImplementedError(kwarg + ' is not yet considered as keyword')
                # check for keywords which can only be a limited set of values
                    ...
                    if not kwargs[kwarg] in KWARG_TYPES[kwarg]:
                        raise ValueError("'" + kwarg + "' needs to be one of " +
                                         "the following: " +
                ...
                # check the data type of the keyword argument
                    ...
                    if not isinstance(kwargs[kwarg], (np.float16, np.float32, np.float64, float)):
                        raise ValueError("'" + kwarg + "' needs to be of " + str(KWARG_TYPES[kwarg]))
                    else:
                        pass  # is valid
                elif KWARG_TYPES[kwarg] is int:
                    if not isinstance(kwargs[kwarg], (np.int8, np.int16, np.int32, np.int64, int)):
                        raise ValueError("'" + kwarg + "' needs to be of " + str(KWARG_TYPES[kwarg]))
                    else:
                        pass  # is valid
    ...
    def getBdgCfg(self, includeSupply=True):
        ...
        if self.cfg:
            ...
            if not os.path.exists(os.path.join(DATA_PATH,'costdata',
                                               self.inputKwargs['costdata'] + '.xlsx')):
                raise ValueError("'costdata' file with name '"
                                 + self.inputKwargs['costdata'] + "' does not exists")
            # add cost data
    ...
    def _get_operation(self, cfg, kwgs):
        ...
        if 'weatherData' in kwgs:
            if not 'weatherID' in kwgs:
                raise ValueError('If weatherData is defined, also ' +
                                 'weatherID has to be defined')
            else:
                cfg['weather'] = kwgs['weatherData']
                cfg['design_T_min'] = cfg['weather'].min()['T']
                cfg['weatherID'] = kwgs['weatherID']
                if (kwgs['longitude'] == KWARG_DEFAULTS['longitude'] or
                            kwgs['latitude'] == KWARG_DEFAULTS['latitude']):
                    warnings.warn('longitude and latitude are set to ' +
                                  'default values. It can cause an error in '
                                  'the solar irration simulation')

        else:
            # get TRY weather and ISO
            cfg['weather'], cfg['design_T_min'], cfg['weatherID'] = \
                get_ISO12831_weather(kwgs['longitude'], kwgs['latitude'],
                                     year=kwgs['year'], cosmo=kwgs['getNetCDF4'])
        ...
        self.IDentries['ComfortZone'] = str(cfg['comfortT_lb']) + '-' + str(cfg['comfortT_ub'])
        if cfg['comfortT_lb'] >= cfg['comfortT_ub'] - 0.5:
            warnings.warn('If the gap is too small, the solver can run into' +
                          ' numerical troubles', UserWarning)
        ...
        if 'n_apartments' in kwgs:
            ...
            cfg['n_apartments'] = self.iwu_bdg.loc[self.IDentries['Shape'], 'n_Apartment']
        ...
        if 'elecLoad' in kwgs:
            if not 'elecLoadID' in kwgs:
                raise ValueError('If elecLoad is defined, also ' +
                                 ' elecLoadID has to be defined')
            else:
                cfg['elecLoad'] = kwgs['elecLoad']
                self.IDentries['elecLoad'] = kwgs['elecLoadID']
                addElecLoad = False
        else:
            self.IDentries['elecLoad'] = 'CREST_' + str(cfg['n_persons']) + 'x' + \
                                         str(cfg['n_apartments'])
            addElecLoad = True
        ...
        state_seed = (str(int(cfg['n_persons'])) +
        ...
        if not self.ignore_profiles:
            cfg = self.get_building_profile(cfg, kwgs, state_seed,
                                            addElecLoad=addElecLoad)

        return cfg

    def get_building_profile(self, cfg, kwgs, state_seed,
                             addElecLoad=True):
        '''
        ...
        seeds = np.random.RandomState(state_seed).randint(0, TOTAL_PROFILE_NUM,
                                                          size=int(cfg['varyoccupancy'] * cfg['n_apartments']))

        # get the profiles
        hh_profiles = get_household_profiles(cfg['n_persons'],
                                             cfg['weather'],
        ...
        for i in range(cfg['varyoccupancy']):
            if n_app == 1:
                ...
                occData = pd.concat(hh_profiles[i * n_app:(i + 1) * n_app],
            ...
            bdg_profiles[i]['Q_ig'] = (occData['AppHeatGain'].values
                                       + occData['OccActive'].values * OccActiveHeatGain
                                       + occData['OccNotActive'].values * OccNotActiveHeatGain) / 1000
            #        bdg_profiles[i]['occ_active'] = occData['OccActive'] > 0.0

            # the share of occupants which is not at home
            bdg_profiles[i]['occ_nothome'] = (n_occs - occData['OccActive'] - occData['OccNotActive']) / n_occs
            ...
            if cfg['hasFirePlace']:
                pot_filename = os.path.join(DATA_PATH, 'results',
                                            'fireplaceprofiles',
                                            'Profile' +
                                            '_apps' + str(int(n_app)) +
                                            '_occ' + str(int(cfg['n_persons'])) +
                                            '_wea' + str(cfg['weatherID']) +
                                            '_seed' + str(state_seed) +
                                            '.csv')
                if os.path.isfile(pot_filename):
                    fireplaceLoad = pd.read_csv(pot_filename, index_col=0,
                                                header = None, parse_dates = True, squeeze = True)
                else:
                    # get oven profile depending on activity and outside temperature   
                    fireplaceLoad = tsm.createWoodFireProfile(cfg['weather']['T'],
    ...
    def _get_form(self, cfg, kwgs):
        ...
        if (('a_ref_app' in kwgs and
                     'n_apartments' in kwgs and
                     'surrounding' in kwgs)
            ...
            surDict = {"B_Alone": 'Detached',
                       "B_N1": 'Semi',
                       "B_N2": 'Terraced'}
            self.iwu_bdg.replace({'Code_AttachedNeighbours': surDict},
                                 inplace=True)
            iwu_sur = (self.iwu_bdg[self.iwu_bdg['Code_AttachedNeighbours']
                                    == kwgs['surrounding']])
            ...
            iwu_bdg = self.iwu_bdg[(self.iwu_bdg['Year1_Building'] <= kwgs['buildingYear']) &
            ...
            is_class = [bdg_class in btype.split('.') for btype in iwu_bdg['Code_BuildingType']]
            iwu_bdg = iwu_bdg[is_class]
            is_eastwest = [bdg_eastwest in btype.split('.') for btype in iwu_bdg['Code_BuildingType']]
            iwu_bdg = iwu_bdg[is_eastwest]
            # test if a building was chosen
            if not iwu_bdg.index.any():
                raise ValueError('Building with this definition does not exist in IWU-Database')
    ...
    def _get_fabric(self, cfg, kwgs):
        if 'ID' in kwgs:
            ...
            cfg['buildingYear'] = self.iwu_bdg.xs(kwgs['ID']).to_dict()['Year2_Building']
        else:
            ...
            if kwgs['refurbished']:
                ...
                warnings.warn('"refurbished" just overwrites the buildingyear from '
                              + str(year_before) + ' to ' + str(year))
            elif kwgs['buildnew']:
                year = 2020
            ...
            iwu_year = self.iwu_bdg[(self.iwu_bdg['Year1_Building'] <= year) &
                                    (year <= self.iwu_bdg['Year2_Building'])]

            # get the one with the most similar reference area
            diff_area = abs(iwu_year['A_C_Ref'] * iwu_year['n_Apartment'] -
        ...
        if cfg['force_refurbishment'] and not cfg['refurbishment']:
            raise ValueError('If "force_refurbishment" is activated, "refurbishment" must be activated as well.')
    ...
    def _get_equipment(self, cfg, kwgs):
        ...
        if 'hasSolarThermal' in kwgs:
            ...
            if (cfg['existingHeatSupply'] == 'Gas boiler' and
                ...
                cfg['solarThermalSize'] = 10. * cfg['n_apartments']
            else:
...
def get_fabric(bdg, iwu_bdg):
    ...
    bdg['U_Window'] = (iwu_bdg['A_Window_1'] * iwu_bdg['U_Window_1'] +
                       iwu_bdg['A_Window_2'] * iwu_bdg['U_Window_2']) / \
                      bdg['A_Window']
    bdg['g_gl_n_Window'] = (iwu_bdg['A_Window_1'] * iwu_bdg['g_gl_n_Window_1'] +
...
def get_shape(bdg, iwu_bdg, a_ref):
    ...
    bdg['A_Window_Horizontal'] = iwu_bdg['A_Window_Horizontal'] \
                                 * ratio / iwu_bdg['n_Storey']
    bdg['A_Window'] = sum(bdg['A_Window_' + di] for di in ['North', 'East', 'South', 'West', 'Horizontal'])
    ...
    for roof in ['Roof_1', 'Roof_2']:
        bdg['A_' + roof] = iwu_bdg['A_' + roof] * (1 + (ratio - 1) / iwu_bdg['n_Storey'])
    for floor in ['Floor_1', 'Floor_2']:
        bdg['A_' + floor] = iwu_bdg['A_' + floor] * (1 + (ratio - 1) / iwu_bdg['n_Storey'])
    for door in ['Door_1']:
        bdg['A_' + door] = iwu_bdg['A_' + door] * (ratio ** 0.5)
    return bdg


def get_household_profiles(n_persons,
                           weather_data, weatherID, seeds=[0],
                           ignore_weather=True,
                           mean_load=True,
    ...
    for seed in seeds:
        ...
        filenames[seed] = os.path.join(DATA_PATH,
                                       'results',
                                       'occupantprofiles',
                                       profile_ID + '.csv')
    ...
    if len(not_existing_profiles) > 1:
        new_profiles = run_district_year(int(n_persons),
                                         2010,
    ...
    # if single profile just create one profile and avoid multiprocessing
        ...
        one_profile = one_household_year(int(n_persons),
                                         2010,
                                        weather_data=weather_data,
                                        get_hot_water=True,
                                        resample_mean=mean_load,)
...
def get_ISO12831_weather(longitude, latitude, year = 2010, cosmo = False):
    ...
    # read weather zones
    wzones = pd.read_csv(os.path.join(DATA_PATH, 'weatherdata',
                                      'ISO12831',
                                      'T_zones_Ger_final.csv'),
                         index_col=0, encoding="ISO-8859-1")

    # get distance to all reference weather station points
    dist = ((wzones['Lat'] - latitude) ** 2
            + (wzones['Lng'] - longitude) ** 2) ** 0.5

    # if distance to next reference position is to high.
    if min(dist) > 5:
        raise NotImplementedError('The weather data is at the moment' +
                                  ' only implemented for Germany')
    ...
    if not cosmo:
        ...
        weather, weatherID =  tsm.readtCosmoNetCDF4(os.path.join(os.environ['DATA_SHARE'],
