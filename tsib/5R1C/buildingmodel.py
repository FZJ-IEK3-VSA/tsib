...
import tsib.5R1C.utils as utils
...
class Building(object):
    ...
    ID_ATTR = ['codeBuildingType','weatherID','design_T_min',
                'refurbishment','n_persons',
                'thermalClass','roofTilt',
                'roofOrientation','comfortT_lb','comfortT_ub',
                'ventControl','nightReduction',
                'capControl','costData', 'elecLoadID']
    ...
    def __init__(self, buildingYear=1976, buildingType='SFH',
             buildingClassification='Gen', weatherData=None, weatherID = None,
             eastOrOverall='N',  annualElecDemand = None,
             n_persons = None, n_apartments = None, latitude=50.0, longitude=8.0,
             roofOrientation = 135., roofTilt = 45.,
             a_ref=None, thermalClass=None, maxLoad = None,
             nightReduction = True, occControl = False, capControl = True,
             ventControl = False, refurbishment = True,
             isRefurbished = False,
             comfortT_lb = 21., comfortT_ub = 24.,
             costdata = 'default_2016', elecLoad = None, elecLoadID = None,
             configurator = None):
        '''
        ...
        if configurator is None:
            ...
            if not isinstance(configurator, config.BuildingConfiguration):
                raise ValueError('configurator needs to be of type ' +
        ...
        self.costdatapath = os.path.join(DATA_PATH, 
                                         'costdata', 
                                         self.cfg['costdata'] + '.xlsx')    
        ...
        # TODO: kick out
        self.ID = (self.IDentries['Fabric'].replace('.','_') +
                    '_apps' + str(self.IDentries['n_apartments']) +
                    '_occ' + str(self.IDentries['n_persons']) +
                    '_lat' + str(self.IDentries['weather'] ) +
                    '_til' + str(round(self.IDentries['roofTilt'] *1e3)/1e3).replace('.','-') +
                    '_ori' + str(round(self.IDentries['roofOrientation'] *1e3)/1e3).replace('.','-') +
                    '_area' + str(round(self.IDentries['A_ref'] *1e3)/1e3).replace('.','-') )
        self.times = self.cfg['weather'].index
        return
    ...
    def _saveResults(self):
        datapath1 = os.path.join(DATA_PATH, 'results', 'buildingprofiles', 
                                self.ID  + '.csv')
        datapath2 = os.path.join(DATA_PATH, 'results', 'buildingstaticresults', 
                                self.ID  + '.csv')
    ...
    def _loadResults(self):
        ...
        if os.path.isfile(datapath1):
            ...
            self.detailedResults.index = pd.to_datetime(self.detailedResults.index,
                                                        utc = True)
            self.static_results = pd.read_csv(datapath2, index_col = 0,squeeze = True, header = None).to_dict()
            return True
        else:
            return False 
    ...
    def _initEnvelop(self, M):
        ...
        for comp in M.bComponents:
            refRaw[comp] = pd.read_excel(self.costdatapath,
                                         sheet_name = comp,
                                         skiprows = [1],
                                         index_col = 0,)
            ...
            if comp in ['Walls','Roof','Floor']:
                refRaw[comp]['U_Value'] = refRaw[comp]['Lambda'] / refRaw[comp]['Thickness']
        ...
        for var in M.bInsul['Walls']:
            ...
            for wall in ['Wall_1','Wall_2','Wall_3']:
                ...
                M.bH['Walls'][var] +=  ( (self._calcUVal(U_vals_insul) * 
                                          self.cfg['A_'+ wall] * 
                                          self.cfg['b_Transmission_'+ wall]) / 1000 )
                                      
                # investment for all walls
                if self.cfg['onlyEnergyInvest']:
                    M.exVarCAPEX[('Walls',var)] += (self.cfg['A_'+wall] *
                                 float(refRaw['Walls'].loc[var,'Investment only energy'].sum()))
                else:
                    M.exVarCAPEX[('Walls', var)] += (self.cfg['A_' + wall] *
                                                 float(refRaw['Walls'].loc[var, 'Investment'].sum()))

            # no operational expenditure for walls
            M.exVarOPEX[('Walls',var)] = 0.
        ...
        for var in M.bInsul['Roof']:
            ...
            for roof in ['Roof_1','Roof_2']:
                ...
                M.bH['Roof'][var] += (self._calcUVal(U_vals_insul) * 
                                      self.cfg['A_'+ roof] * 
                                      self.cfg['b_Transmission_'+ roof] / 1000)
        ...
        for var in M.bInsul['Floor']:
            ...
            for floor in ['Floor_1', 'Floor_2']:
                ...
                if isinstance(U_vals_insul, (np.float64)):
                    ...
                    U_vals_insul = np.append(U_vals_insul.values, self.cfg['U_' + floor])

                # add heat resistance for each floor [kW/K]
                M.bH['Floor'][var] += (self._calcUVal(U_vals_insul) *
                                      self.cfg['A_' + floor] *
                                      self.cfg['b_Transmission_' + floor] / 1000)

                # investment for all Floor
                M.exVarCAPEX[('Floor', var)] += self.cfg['A_' + floor] * float(refRaw['Floor'].loc[var, 'Investment'].sum())

            M.exVarOPEX[('Floor', var)] = 0.
        ...
        for var in M.bInsul['Windows']:
            M.bH['Windows'][var] = (self.cfg['A_Window']) * float(refRaw['Windows'].loc[var,'U_Value'])   /1000
            M.bU['Windows'][var] = float(refRaw['Windows'].loc[var,'U_Value']) /1000
            M.bg_gl[var] = float(refRaw['Windows'].loc[var,'g_gl'])
            M.exVarCAPEX[('Windows',var)] = (self.cfg['A_Window'] * 
                                 float(refRaw['Windows'].loc[var,'Investment']))
            M.exVarOPEX[('Windows',var)] = 0.
            M.exVarLifetime[('Windows', var)] = self.lifetime

        # VENTILATION
        # heat capacity of the whole air
        C_air = self.cfg['A_ref'] * self.cfg['h_room']  * self.CONST['rho_air']  * self.CONST['C_air']
        ...
        for var in M.bInsul['Ventilation']:
            ...
            M.bH['Ventilation'][var] = C_air * (self.cfg['n_air_use'] * (1 - float(refRaw['Ventilation'].loc[var, 'Recovery rate']))  +
                                                self.cfg['n_air_infiltration']) / 3600   # [kW/K]
            M.exVarCAPEX[('Ventilation', var)] = (self.cfg['A_ref'] *
                             float(refRaw['Ventilation'].loc[var,'Investment']))
            M.exVarOPEX[('Ventilation', var)] = float(refRaw['Ventilation'].loc[var,'OPEX-Fix'])
            M.exVarLifetime[('Ventilation', var)] = float(refRaw['Ventilation'].loc[var,'Lifetime'])
    ...
    def _initControl(self, M):
        ...
        costRaw = pd.read_excel(self.costdatapath,
                                 sheet_name = 'Control',
                                 skiprows = [1],
                                 index_col = 0,)
        ...
        for var in costRaw.index:
            M.exVarCAPEX[('Control', var)] = (self.cfg['A_ref'] * float(costRaw.loc[var,'Investment spec'])
    ...
    def _init5R1C(self, M):
        ...
        # variant possible
        bClass_f_lb = {'very light': 0.0, 'light': 95.0, 'medium': 137.5,
                       'heavy': 212.5, 'very heavy': 313.5}
        bClass_f_ub = {'very light': 95.0, 'light': 137.5, 'medium': 212.5,
                       'heavy': 313.5, 'very heavy': 313.5*2}
        bClass_f_a = {'very light': 2.5, 'light': 2.5, 'medium': 2.5,
                      'heavy': 3.0, 'very heavy': 3.5}
        ...
        self.cfg['c_m'] = (bClass_f_lb[self.cfg['thermalClass']] +
                          bClass_f_ub[self.cfg['thermalClass']])/2.0
        # internal heat capacity [kWh/K]
        M.bC_m = self.cfg['A_ref'] * self.cfg['c_m']/ 3600.
        ...
        surf_az = {'North': 0.0 + ro, 'East': 90.0 + ro, 'South': 180. + ro, 
                   'West': 270.0 + ro, 'Horizontal': 180.0, 
                   'Roof 1': 90. + ro, 'Roof 2': 270. + ro, }
        surf_tilt = {'North': 90.0, 'East': 90.0, 'South': 90.0, 'West': 90.0,
                   'Horizontal': 0.0, 'Roof 1': rt, 'Roof 2': rt, }
        F_r = {'North': 0.5, 'East': 0.5, 'South': 0.5, 'West': 0.5,
               'Horizontal': 1.0}  
        ...
        window_area_s = {di: self.cfg['A_Window_' + di] * self.cfg['F_sh_vert']
                       for di in ['North', 'East', 'South', 'West']}
        window_area_s['Horizontal'] = self.cfg['A_Window_Horizontal'] * self.cfg['F_sh_hor']
        
        # get relevant window area for thermal irradiance
        window_area_t = sum( self.cfg['A_Window_' + key] * F_r[key] 
                         for key in F_r)

        # get solar radiation on window area
        irrad_on_windows = self.irrad_surf.mul(pd.Series(window_area_s)).sum(axis=1).values

        # solar gain time series depending on the investment decision
        for var in M.bInsul['Windows']:
            # thermal radiation [kW] (Schuetz et al. 2017 - eq. 13)
            thermal_rad = (window_area_t * M.bConst['h_r'] *
                            M.bU['Windows'][var]  * M.bConst['R_se'] *
                            M.bConst['delta_T_sky'])
            M.profiles['bQ_sol_Windows' + var] = (irrad_on_windows *
                               (1.0 - self.cfg['F_f']) *self.cfg['F_w'] *
                               M.bg_gl[var] - thermal_rad)

        
        # WALLS
        mean_ver_irr = self.irrad_surf.loc[:, ['North', 'East','South', 'West']].mean(axis=1).values
        for var in M.bInsul['Walls']:
            # thermal radiation [kW] (Schuetz et al. 2017 - eq. 13)
            thermal_rad = (M.bH['Walls'][var] * M.bConst['h_r'] *
                             M.bConst['R_se'] * M.bConst['delta_T_sky'])
            solar_radiation = (mean_ver_irr * M.bH['Walls'][var] * 
                                self.cfg['F_sh_vert'] * M.bConst['R_se'] *
                                M.bConst['alpha'])
            M.profiles['bQ_sol_Walls' + var] = ( solar_radiation
                                  - thermal_rad) 
        ...
        for var in M.bInsul['Roof']:
            ...
            thermal_rad = (M.bH['Roof'][var] * M.bConst['h_r'] *
                              M.bConst['R_se'] * M.bConst['delta_T_sky'])
            solar_radiation = (mean_roof_irr * M.bH['Roof'][var] * 
                                self.cfg['F_sh_hor'] * M.bConst['R_se'] *
                                M.bConst['alpha'])
            M.profiles['bQ_sol_Roof' + var] = ( solar_radiation
                                  - thermal_rad) 
        return M
    ...
    def _calcRadiation(self, surf_az, surf_tilt):
        ...
        SOL_POS = pvlib.solarposition.get_solarposition(self.cfg['weather'].index,
                                                        self.cfg['latitude'],
                                                        self.cfg['longitude'])
        ...
        for key in surf_az:
            ...
            total = irr.get_total_irradiance(surf_tilt[key], surf_az[key],
                                    ...
                                    model='perez', surface_type='urban')
    ...
    def _initOpti(M):
        ...
        if not hasattr(M,'profiles'): M.profiles=utils.PowerDict()
        if not hasattr(M,'profilesEval'): M.profilesEval=utils.PowerDict()
        if not hasattr(M,'exVarIx'): M.exVarIx = []
        if not hasattr(M,'exVarActive'): M.exVarActive = []
        if not hasattr(M,'exVarInActive'): M.exVarInActive = []
        if not hasattr(M,'insulIx'): M.insulIx = []
        if not hasattr(M,'exVarCost'): M.exVarCost = utils.PowerDict()
        if not hasattr(M,'exVarCAPEX'): M.exVarCAPEX = utils.PowerDict()
        if not hasattr(M,'exVarOPEX'): M.exVarOPEX = utils.PowerDict()
        if not hasattr(M,'exVarLifetime'): M.exVarLifetime = utils.PowerDict()
        if not hasattr(M,'bConnectedHeat'): M.bConnectedHeat = []
        if not hasattr(M,'bConnectedElec'): M.bConnectedElec = []
        if not hasattr(M,'bConnectedCool'): M.bConnectedCool = []
        return M
    ...
    def _addOpti(self,M):
        ...
        if hasattr(M, 'bMaxLoad'):
            raise NotImplementedError('At the moment only single building' +
        ...
        for dec in M.exVarIx:
            aFactor = ((1 + WACC) ** M.exVarLifetime[dec]) * WACC / (
                ((1 + WACC) ** M.exVarLifetime[dec]) - 1) if WACC > 0 else 1. / M.exVarLifetime[dec]
            M.exVarCost[dec] = M.exVarCAPEX[dec] * (aFactor + M.exVarOPEX[dec]) * M.optInterval/8760
        ...
        for comp in M.bInsul:
            ...
            M.bm_q[comp] = {}
            for dec in M.bInsul[comp]:
                M.bM_q[comp][dec] = M.bH[comp][dec] * (M.bT_comf_ub
                                                        - (self.cfg['weather']['T'].min()-10))
                M.bm_q[comp][dec] = M.bH[comp][dec] * (M.bT_comf_lb
                                                        - (self.cfg['weather']['T'].max()+10))
    ...
    def _addCons(M):
        ...
        def auxiEq3(M,dec_w,comp_w,dec_x,comp_x):
            return M.bP_X[(dec_w,comp_w,dec_x,comp_x)] >= M.exVars[(dec_w,comp_w)] + M.exVars[(dec_x,comp_x)] - 1
        ...
        def gainMassNode(M,t1,t2):
            return (M.bQ_m[t1,t2] == 0.5 * M.bA_m / M.bA_tot * M.profiles['bQ_ig'][t1,t2] + 
                    M.bA_f / M.bA_tot * sum( M.profiles['bQ_sol_'+comp+dec][t1,t2] * 
                    M.exVars[(comp,dec)] for (comp,dec) in M.bX_solar) )
        M.bGainMassNodeCon = pyomo.Constraint(M.timeIndex, rule=gainMassNode)
        # (Schuetz et al. 2017 - eq. 16)
        def gainSurNode(M,t1,t2):
            return (M.bQ_st[t1,t2] == 
                (1 - 1/ M.bConst['h_ms']/ M.bA_tot * sum(M.bU['Windows'][dec] * 
                 M.exVars[('Windows',dec)] for (comp,dec) in M.bX_windows)) *
                 (0.5 * M.profiles['bQ_ig'][t1,t2]) 
                    - 1/ M.bConst['h_ms']/ M.bA_tot * 
                    sum(M.bH['Windows'][dec_w] * 
                    sum( M.profiles['bQ_sol_'+comp_x+dec_x][t1,t2] * 
                    M.bP_X[(comp_w,dec_w,comp_x,dec_x)] 
                    for (comp_x,dec_x) in M.bX_solar)
                    for (comp_w,dec_w) in M.bX_windows )                    
                    + sum( M.profiles['bQ_sol_'+comp+dec][t1,t2] * 
                    M.exVars[(comp,dec)] for (comp,dec) in M.bX_solar)
                    - M.bQ_m[t1,t2])
        ...
        def heatFlowZeroUB(M,t1,t2,comp,dec):
            return (M.bQ_comp[comp,dec,t1,t2] <= 
                    M.exVars[comp,dec] * M.bM_q[comp][dec])
        M.heatFlowZeroUBCon = pyomo.Constraint(M.timeIndex, M.insulIx,
                                               rule = heatFlowZeroUB)
        # (Schuetz et al. 2017 - eq. 23-25)
        def heatFlowZeroLB(M,t1,t2,comp,dec):
            return (M.bQ_comp[comp,dec,t1,t2] >= 
                    M.exVars[comp,dec] * M.bm_q[comp][dec])
        M.heatFlowZeroLBCon = pyomo.Constraint(M.timeIndex, M.insulIx,
                                               rule = heatFlowZeroLB)
        ...
        def heatFlowActiveUB(M,t1,t2,comp,dec):
            return (M.bH[comp][dec] * (M.bT_m[t1,t2] - M.profiles['T_e'][t1,t2] )
                    - M.bQ_comp[comp,dec,t1,t2] <= 
                    (1 - M.exVars[comp,dec]) * M.bM_q[comp][dec])
        M.heatFlowActiveUBCon = pyomo.Constraint(M.timeIndex, M.insulIx,
                                               rule = heatFlowActiveUB)
        # (Schuetz et al. 2017 - eq. 8, 23-25)
        def heatFlowActiveLB(M,t1,t2,comp,dec):
            return (M.bH[comp][dec] * (M.bT_m[t1,t2] - M.profiles['T_e'][t1,t2] )
                    - M.bQ_comp[comp,dec,t1,t2] >= 
                    (1 - M.exVars[comp,dec]) * M.bm_q[comp][dec])
        M.heatFlowActiveLBCon = pyomo.Constraint(M.timeIndex, M.insulIx,
                                               rule = heatFlowActiveLB)
        ...
        if not M.bVentControl:
            ...
            def ventFlowLin(M,t1,t2):
                return M.bQ_ve[t1,t2] == (M.bH_ve_con[t1,t2] * ( M.bT_air_WP - M.profiles['T_e'][t1,t2])
...
#        
            ...
            def ventMean(M, d):
                return (sum(M.bH_ve_con[1,t] for t in range(d*M.stepsPerDay, (d+1) * M.stepsPerDay)) 
...
#        M.bH_air_inf = V_air * rho_air * C_air * self.cfg['n_air_infiltration']  / 3600
        ...
        if M.hasTypPeriods:
            def balMassNode(M,t1,t2):
                return (M.bH_ms * ( M.bT_m[t1,t2] - M.bT_s[t1,t2])
                        + sum(M.bQ_comp[comp,dec,t1,t2] for (comp,dec) in M.bX_opaque)
                        + M.bH_door * ( M.bT_m[t1,t2] - M.profiles['T_e'][t1,t2] )
                        == M.bQ_m[t1,t2] - M.bC_m * (M.bT_m[t1,t2+1] 
                        - M.bT_m[t1,t2]) / M.stepSize)
            M.balMassNodeCon = pyomo.Constraint(M.typPeriodIx, M.intraIx[:-1], rule = balMassNode)
        
            def balMassNodeStartEnd(M, period):
                return (M.bH_ms * ( M.bT_m[period,M.intraIx[-1]] - M.bT_s[period,M.intraIx[-1]])
                        + sum(M.bQ_comp[comp,dec,period,M.intraIx[-1]] for (comp,dec) in M.bX_opaque)
                        + M.bH_door * ( M.bT_m[period,M.intraIx[-1]] - M.profiles['T_e'][period,M.intraIx[-1]] )
                        == M.bQ_m[period,M.intraIx[-1]] - M.bC_m * (M.bT_m[period,M.intraIx[0]]
                        - M.bT_m[period,M.intraIx[-1]]) / M.stepSize)
            M.balMassNodeStartEndCon = pyomo.Constraint(M.typPeriodIx,rule = balMassNodeStartEnd)
        else:
            def balMassNode(M,t1,t2):
                return (M.bH_ms * ( M.bT_m[t1,t2] - M.bT_s[t1,t2])
                        + sum(M.bQ_comp[comp,dec,t1,t2] for (comp,dec) in M.bX_opaque)
            ...
            def balMassNodeStartEnd(M):
                return (M.bH_ms * ( M.bT_m[M.timeIndex[-1]] - M.bT_s[M.timeIndex[-1]])
                        + sum(M.bQ_comp[comp,dec,M.timeIndex[-1]] for (comp,dec) in M.bX_opaque)
                        + M.bH_door * ( M.bT_m[M.timeIndex[-1]] - M.profiles['T_e'][M.timeIndex[-1]] )
                        == M.bQ_m[M.timeIndex[-1]] - M.bC_m * (M.bT_m[M.timeIndex[0]] 
                        - M.bT_m[M.timeIndex[-1]]) / M.stepSize)
            M.balMassNodeStartEndCon = pyomo.Constraint(rule = balMassNodeStartEnd)
        
        def balSurfaceNode(M,t1,t2):
            return (M.bH_ms * ( M.bT_s[t1,t2] - M.bT_m[t1,t2])
        ...
        def balAirNode(M,t1,t2):
            return (sum(M.bQ_comp[comp,dec,t1,t2] for (comp,dec) in M.bX_vent)
                    + M.bH_is * ( M.bT_air[t1,t2] - M.bT_s[t1,t2])
                    == M.bQ_st[t1,t2] 
        ...
        M.bDesignAdjust = {'Floor': 1.45,
                           'Walls': 1.0,
                           'Roof': 1.0,
                           'Windows': 1.0,
                           'Ventilation': 1.0, }
        def designLoad(M):
            return M.bQ_des == (( M.bH_door +
                               sum( M.bH[comp][dec] * M.bDesignAdjust[comp] * M.exVars[comp,dec]
        ...
        if M.bRefurbishment:
            ...
            def controlOrder1(M):
                return M.exVars[('Control','Occupancy')] <= M.exVars[('Control','SmartThermostat')]
            M.controlOrder1 = pyomo.Constraint(rule = controlOrder1)
            def controlOrder2(M):
                return M.exVars[('Control','Occupancy')] <= M.exVars[('Control','NightReduction')]
            M.controlOrder2 = pyomo.Constraint(rule = controlOrder2)

        # define upper bound of the temperature
        def temperatureUB(M, t1, t2):
            return M.bT_air[t1, t2] <= (M.bT_comf_lb + (M.bT_comf_ub - M.bT_comf_lb) * M.exVars[('Control','SmartThermostat')]
                           - (M.bT_comf_ub - 30.0  ) * M.profiles['bOccNotHome'][t1,t2] * M.exVars[('Control','Occupancy')])
        ...
        def temperatureLB(M, t1, t2):
            return M.bT_air[t1, t2] >= (M.bT_comf_lb 
                           - (M.bT_comf_lb - 18.0  ) * M.profiles['bOccSleeping'][t1,t2] * M.exVars[('Control','NightReduction')]
                           - (M.bT_comf_lb - 14.0  ) * M.profiles['bOccNotHome'][t1,t2] * M.exVars[('Control','Occupancy')])
        ...
        def electricityBalanceRule(M,t1,t2):
            return (sum(M.connectVars[c,t1,t2] for c in M.bConnectedElec) == M.profiles['bElecLoad'][t1,t2])
        M.electricityBalanceCon = pyomo.Constraint(M.timeIndex, rule=electricityBalanceRule)
    ...
    def _readResults(self,M):
        ...
        '''            
        self.detailedResults['Heating Load'] = np.array([sum(M.connectVars[c,t].value for c in M.bConnectedHeat) for t in M.fullTimeIndex])
        self.detailedResults['Cooling Load'] = np.array([sum(M.connectVars[c,t].value for c in M.bConnectedCool) for t in M.fullTimeIndex])
        self.detailedResults['T_air'] = np.array([M.bT_air[t].value for t in M.fullTimeIndex])
        self.detailedResults['T_s'] = np.array([M.bT_s[t].value for t in M.fullTimeIndex])
        self.detailedResults['T_m'] = np.array([M.bT_m[t].value for t in M.fullTimeIndex])
        self.detailedResults['T_e'] = np.array([M.profiles['T_e'][t] for t in M.fullTimeIndex])
        ...
        for dec in M.exVarIx:
            if M.exVars[dec].stale:
                if M.exVars[dec].lb == M.exVars[dec].ub:
                    ...
                    warnings.warn('Stale value in result of ' + str(dec) +
                                  ' detected. Result is set to the lb '
                                  'of the variable', UserWarning)
                    M.exVars[dec].value = M.exVars[dec].lb
            self.detailedRefurbish[dec] = {}
            self.detailedRefurbish[dec]['Capacity'] = M.exVars[dec].value
            self.detailedRefurbish[dec]['FixCost'] = M.exVarCost[dec] * M.exVars[dec].value
            self.detailedRefurbish[dec]['CAPEX'] = M.exVarCAPEX[dec] * M.exVars[dec].value
    ...
    def calcDesignHeatLoad(self):
        ...
        b = self.cfg 
        designHeatLoad = ((b['A_Roof_1'] * b['U_Roof_1'] * b['b_Transmission_Roof_1']
                 ...
                 + (b['A_ref'] * b['h_room'] * 1.2 * 1006 
                    * (b['n_air_infiltration'] + b['n_air_use']) / 3600)) 
                * ( 22.917 - self.cfg['design_T_min'] )
                 ...
                 * ( 22.917 - self.cfg['design_T_min'] ) * 1.45 ))
...
#        self.design_H_heat = self.design_Q / (self.cfg['T_sup'] - self.design_T_return)
    ...
    def sim5R1C(self, solver = None, tee = True):
        ...
        M.connectIndex = [('Heat','Building'), 
                          ('Electricity','Building'),
                          ('Cool','Building')]
        M.connectVars = pyomo.Var(M.connectIndex,
                                  M.timeIndex, 
        ...
        if not M.hasTypPeriods: 
            for profile_name in M.profiles:
                M.profiles[profile_name] = {timeIndex:M.profiles[profile_name][i] for i,timeIndex in enumerate(M.timeIndex) }
        ...
        def maxHeatingLoad(M,t1,t2):
            return (sum(M.connectVars[c,t1,t2] for c in M.bConnectedHeat) 
                    - M.bMaxLoadViolation <= M.bMaxLoad)
        M.maxHeatingLoadCon = pyomo.Constraint(M.timeIndex, rule=maxHeatingLoad)
        
        def maxCoolingLoad(M,t1,t2):
            return (sum(M.connectVars[c,t1,t2] for c in M.bConnectedCool)
                    - M.bMaxLoadViolation <= M.bMaxLoad)
        ...
        # objective function
        def minLoad(M):
            return  (sum(sum(M.connectVars[c,t] for c in M.bConnectedCool)  for t in M.timeIndex) * M.bCoolCost+ 
                     sum(sum(M.connectVars[c,t] for c in M.bConnectedHeat)  for t in M.timeIndex) * M.bHeatCost+ 
                     sum(sum(M.connectVars[c,t] for c in M.bConnectedElec)  for t in M.timeIndex) * M.bEectricityCost+ 
        ...
        optprob.options = utils.manageSolverOpts(solver, {'Threads':1, 'LogFile':''}) #, 'LogToConsole':0

        # solve
        self.opt_results = optprob.solve(M, tee = tee)
        
        if not M.bMaxLoadViolation.value is None:
            if M.bMaxLoadViolation.value > 0.:
                warnings.warn('Maximal heat load exceeded by ' + str(round(M.bMaxLoadViolation.value,3)) + ' kW', UserWarning)
    ...
    def getLoad(self):
        ...
        if resultsExist:
            ...
            if self.cfg['refurbishment']:
                befRef = True
                warnings.warn('For the simulation the refurbishment decisions' + 
                              ' are deactivated', UserWarning)
    ...
    def toCSV(self, filename = 'result'):
        ...
        filename: str, optional, default: "result"
            Filepath and name.
        '''
        if filename[-4:] is '.csv': filename = filename[:-4]
    ...
    def readCSV(self, filename = 'result'):
        ...
        '''
        if filename[-4:] is '.csv': filename = filename[:-4]
        self.detailedResults = pd.read_csv(filename + "_dynamic.csv",sep=',',index_col=0,
                                 parse_dates=True)
...
#    cfg = bdg.getBdgCfg()
    example = Building( configurator = bdg,
                       refurbishment = False)
...
#                 n_persons = 4, refurbishment = False, ventControl = False,
    ...
    example.sim5R1C(solver="gurobi", tee=True)
    example.detailedResults['5R1C Cooling Load T=20-26'] = -example.detailedResults['Cooling Load']
    example.detailedResults['5R1C Heating Load T=20-26'] = example.detailedResults['Heating Load']


    example.detailedResults[['5R1C Heating Load T=20-26',
                             '5R1C Cooling Load T=20-26']].plot()
    plt.show()
