...
def calcGHI(timeSeries,longitude,latitude):
        ...
        '''        
        solarPos = pvlib.solarposition.get_solarposition(timeSeries.index, latitude,
                                                       longitude)
        if 'DNI' in timeSeries:
            timeSeries['GHI'] = timeSeries['DHI'] + timeSeries['DNI'] * math.cos(math.radians(solarPos['apparent_zenith']))
...
def readTMY(filepath=os.path.join('TMY', 'Germany DEU Koln (INTL).csv')):
    ...
    '''
    # get data
    data = pd.read_table(os.path.join(DATA_PATH, 'weatherdata', filepath), 
                        skiprows=([0,1]), delimiter=',')
    data.index = pd.date_range('2010-01-01 00:30:00', periods=8760, freq='H', tz='Europe/Berlin')
    data=data.rename(columns = {'Beam':'DNI','Diffuse':'DHI','Tdry':'T','Wspd':'WS'}) 
    location_data = pd.read_table(os.path.join(DATA_PATH, 'profiles', filepath), 
                        nrows = 1, delimiter=',')
    
    location = {'name':location_data['City'].values[0],
...
def readLindenberg(filepath, year = 2000):
    ...
    for month in range(1,13):
        read1 = pd.read_table(os.path.join(filepath,'Irradiance','LIN_radiation_' 
                                           + str(year) + '-' 
                                           + '{:02d}'.format(month) + '.tab'), 
                              header = 31, index_col = 0)
        raw1 = raw1.append(read1)
        read2 = pd.read_table(os.path.join(filepath,'Weather','LIN_SYNOP_'
                                           + str(year) + '-' 
                                           + '{:02d}'.format(month) + '.tab'), 
                              header = 31, index_col = 0)
    ...
    data.index = data.index.tz_localize(pytz.utc).tz_convert('Europe/Berlin').shift(-1,freq='H')
    
    # rename relevant columns
    data=data.rename(columns = { 'DIR [W/m**2]': 'DNI','DIF [W/m**2]':'DHI', 
                                'SWD [W/m**2]':'GHI','dd [deg]':'Wdir', 
                                'ff [m/s]':'Wspd', 'TTT [°C]':'DryBulb'}) 
    ...
    if year == 2000:
        ...
        warnings.warn('Other years than 2010 are not validated in terms ' +
                      'of missing data')
...
def readTRY(try_num = 4, year = 2010):
    ...
    filepath = os.path.join(DATA_PATH, 'weatherdata', 'TRY',
    ...
    # else read from .dat and calculate DNI etc.
        ...
        # get data
        data = pd.read_table(filepath + '.dat', 
                            sep='\s+', 
                            skiprows=([i for i in range(0,36)]+[37]))
        data.index = pd.date_range('2010-01-01 00:30:00', periods=8760, freq='H', tz='Europe/Berlin')
...
def readtCosmoNetCDF4(datapath, lon, lat, year):
    ...
    data = res.weather.CosmoSource([os.path.join(datapath,str(year), '*.nc'),],
                                   bounds = (lon, lat), verbose=False)
    ...
    # get data
        ...
        time_index = pd.date_range(start = str(year) + '-01-01 00:30:00',
                                   end = str(year) + '-12-31 23:30:00', freq='H', tz='Europe/Berlin')
...
def calculateDNI(directHI, lon, lat, zenith_tol = 87.):
    """
    ...
    DNI: pd.Series
    """
    solarPos = pvlib.solarposition.get_solarposition(directHI.index, lat,
                                                     lon)
...
def TRY2TM2(trydata):
    ...
    '''
    trydata['Year']=2010
    trydata=trydata.rename(columns = {'MM':'Month','DD':'Day', 'HH':'Hour',
                              'T':'Tdry','p':'Pres','WR':'Wdir',
                              'WS':'Wspd'})
    trydata['Tdew'] = trydata['Tdry']
    
    return  trydata[['Year','Month','Day','Hour','GHI','DHI','Tdry','Tdew','Pres',
                'Wdir','Wspd']]
...
def TRY2TMY(trydata):
    ...
    names to the TM2 format.
    '''
    return trydata.rename(columns = {'MM':'Month','DD':'Day', 'HH':'Hour',
                          'T':'DryBulb','p':'Pressure','WR':'Wdir',
                          'WS':'Wspd'})

    
def simPV_SAM(weatherData, tilt = 30, azimuth = 180, losses = 0.14, 
              efficiency = 0.17 ):
    '''
    ...
    ssc.data_set_number(dat, 'inv_eff', 100 ); # just want to have the DC power
...
def simPV_PV_Lib(tmy_data, surface_tilt = 30, surface_azimuth = 180, albedo = 0.2,
                 latitude = 55, longitude = 7, losses = 0.1, load_module_data = False, 
                 module_name = 'Hanwha_HSL60P6_PA_4_250T__2013_', integrateInverter = True,
    ...
    dni_extra = pd.Series(pvlib.irradiance.get_extra_radiation(tmy_data.index), index=tmy_data.index) # automatic pd time series in future pvlib version
    # calculate airmass
    airmass = pvlib.atmosphere.get_relative_airmass(solpos['apparent_zenith'])
    # use pereze model to calculate the plane of array diffuse sky radiation
    poa_sky_diffuse = pvlib.irradiance.perez(surface_tilt, surface_azimuth,
    ...
    if load_module_data:
        ...
        modules = pd.read_csv(os.path.join(DATA_PATH, 'additionaldata', 'pvmodules',
                                           'sandia_modules.csv'),
                              index_col = 0)
        module = modules[module_name]
        module = pd.to_numeric(module,errors='coerce')

        inverters = pd.read_csv(os.path.join(DATA_PATH, 'additionaldata', 'inverters',
                                           'sandia_modules.csv'),
                              index_col = 0)
        inverter = inverters[inverter_name]
    ...
    sapm_irr = pvlib.pvsystem.sapm_effective_irradiance(module=module, poa_direct=poa_irrad.poa_direct,
                                                        poa_diffuse=poa_irrad.poa_diffuse,
                                                        airmass_absolute=airmass, aoi=aoi)
    # calculate pv performance
    sapm_out = pvlib.pvsystem.sapm(sapm_irr, module=module,  temp_cell = pvtemps['temp_cell'],)
    ...
    if integrateInverter:
        ...
        inv_load = pvlib.pvsystem.snlinverter(inverter=inverter, v_dc=sapm_out['v_mp'], p_dc=sapm_out['p_mp'])
        # load in [kW/kWp]
    ...
    space_coverage = module.ix['Area']/peak_load*1000.
    return specific_load, space_coverage


def simSolarThermal(tmy_data, surface_tilt = 30, surface_azimuth = 180, albedo = 0.2,
                 latitude = 55, longitude = 7, module_temp = 45., 
                 c_0 = 0.791, c_1 = 4.47, c_2 = 0.0069):
    '''
    ...
    poa_ground_diffuse = pvlib.irradiance.get_ground_diffuse(surface_tilt, 
                                                        tmy_data['GHI'], albedo=albedo)
    # calculate angle of incidence
    aoi = pvlib.irradiance.aoi(surface_tilt, surface_azimuth, 
                               solpos['apparent_zenith'], solpos['azimuth'])
    
    # calculate plane of array irradiance
    poa_irrad = pvlib.irradiance.poa_components(aoi, tmy_data['DNI'], 
...
def createWoodFireProfile(temperature, occ_act, n_ovens = 1, T_oven_on = 5, 
                          t_cool = 5., fullloadSteps = 450, seed = None):
    '''
    ...
    relCold = (T_oven_on - temperature) / \
        (T_oven_on - temperature.min())
    ...
    if abs(profile.sum()-fullloadSteps) > 100:
        warnings.warn('Fullload hour deviation is higher than 100. ' +
...
#    y = (timeSeries.values.astype(float) - timeSeries.min()) / (timeSeries.max() - timeSeries.min()) 
    ...
    # plot results
    color =  np.array([0., 83., 130.])/255
    ac_color = np.array([192., 0., 0., 255.])/255
#    plt.figure()
...
#            if xf_val>1./(8750.*T.total_seconds()):
    #            raise ValueError()
            plt.scatter(xf_val*3600,spd[ii],marker = 'o', s = 20, edgecolors=ac_color,facecolors='None',linewidth =1 )
            ax.text( xf_val*3600 , ylim/40 + spd[ii],
                    str(int(round(1/(xf_val*T.total_seconds()))))+' h',
                    ha='left', va='bottom',color=ac_color)
#            else:
...
#    ax.set_xlabel('Frequency [1/h]')
    ax.set_xlim([1./ ( T.total_seconds()/3600 * (N+1)) /2, 3600/T.total_seconds()*0.5])
    ax.set_xscale('log')
    ax.set_yscale('log')
...
#    htw_data = readHTWProfiles()
    ...
    specific_load_se, space_coverage_se = simPV_PV_Lib(tmy_data, surface_tilt = 30, surface_azimuth = 135, albedo = 0.2,
