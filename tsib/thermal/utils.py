            ...
            for column in resultManager.nodes.columns:
                if "Heat pump" in column and not "Heat pump" == column:
                    if isinstance(resultManager.nodes.index, pd.MultiIndex):
                        resultManager.nodes.loc[
                            pd.IndexSlice[:, "Capacity"], column
                        ] = 0
                    else:
        ...
        if not onlyNodes:
            for column in resultManager.flows.columns:
                ...
                if node in FromTo[0] and not node == FromTo[0]:
                    ...
                    resultManager.flows.loc[
                        :, node + " to " + FromTo[1]
                    ] += resultManager.flows.loc[:, column]
            ...
            for column in resultManager.loads.columns:
                if node in column and not node == column:
                    ...
                    resultManager.loads.loc[:, node] += resultManager.loads.loc[
                        :, column
                    ]
...
def renameComponent(resultManager, oldName, newName, onlyNodes=False):
    ...
    if not onlyNodes:
        ...
        for column in resultManager.flows.columns:
            FromTo = column.split(" to ")
            if oldName == FromTo[0]:
                resultManager.loads.rename(
                    columns={str(column): str(column).replace(oldName, newName)},
                    inplace=True,
                )
    return resultManager
...
def getGridExchange(resultManager):
    ...
    gridExchange = -resultManager.flows[
        [
            "Photovoltaic 1 to FiTPV",
    ...
    gridExchange = (
        gridExchange
        + resultManager.flows["Electricity supply to AC Node"]
...
OWN_CMAP = make_colormap(
    [
        np.array([2.0, 61.0, 100.0]) / 255 * 0.8,
        np.array([2.0, 61.0, 100.0]) / 255,
        0.1,
        np.array([2.0, 61.0, 100.0]) / 255,
        np.array([2.0, 61.0, 100.0]) / 255 * 1.3,
        0.25,
        np.array([2.0, 61.0, 100.0]) / 255 * 1.3,
        np.array([30.0, 150.0, 81.0]) / 255 * 1.3,
        0.7,
        np.array([30.0, 150.0, 81.0]) / 255 * 1.3,
        np.array([250.0, 235.0, 90.0]) / 255,
    ]
)

OWN_CMAP_2 = make_colormap(
    [
        np.array([2.0, 61.0, 100.0]) / 255,
        np.array([2.0, 61.0, 100.0]) / 255 * 1.3,
        0.25,
...
CMAP_DISS = make_colormap(
    [
        np.array([2.0, 61.0, 100.0]) / 255,
        np.array([20.0, 129.0, 129.0]) / 255,
        0.25,
        np.array([20.0, 129.0, 129.0]) / 255,
        np.array([108.0, 139.0, 70.0]) / 255,
        0.5,
        np.array([108.0, 139.0, 70.0]) / 255,
        np.array([250.0, 150.0, 90.0]) / 255,
        0.75,
        np.array([250.0, 150.0, 90.0]) / 255,
        np.array([255.0, 192.0, 0.0]) / 255,
    ]
)
